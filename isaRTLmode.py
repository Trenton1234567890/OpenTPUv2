...
OPCODE2BIN = {
    ...
    # New operations:
    'POOL':    (0x8, UB_ADDR_SIZE, UB_ADDR_SIZE, 1),  # Pooling: source and destination both use UB addressing
    'SOFTMAX': (0x9, UB_ADDR_SIZE, UB_ADDR_SIZE, 1),  # Softmax: source and destination both use UB addressing
    'ADD':     (0xA, UB_ADDR_SIZE, UB_ADDR_SIZE, 1),  # Add: source and destination both use UB addressing, ideally because we overwrite the original matrix with the addition results
}
...
# Add other necessary functions or updates for the decoder if required.
