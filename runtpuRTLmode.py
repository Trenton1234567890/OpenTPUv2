...
import argparse
import numpy as np
#from tpuRTLmode import helperfuncs
import pyrtl
#set_debug_mode()

from tpuRTLmode import *
...
din = {
    ...
    host_memory_data_in : 0,
    #addition_matrix_in : 0, was previously using to manually add biases
}
...
while True:
    ...
    if sim.inspect(host_memory_read_enable):
        raddr = sim.inspect(host_memory_read_addr)
        if raddr in hostmem:
            d[host_memory_data_in] = hostmem[raddr]

    # Write host memory signal
    if sim.inspect(host_memory_write_enable):
        waddr = sim.inspect(host_memory_write_addr)
        wdata = sim.inspect(host_memory_data_out)
        hostmem[waddr] = wdata

    # Read weights memory signal
    if sim.inspect(weights_dram_memory_read):
        weightaddr = sim.inspect(weights_dram_read_addr)
...
for wire in pyrtl.working_block().wirevector_set:
     if wire.name.startswith("tmp"):
        wire.name = "_" + wire.name

with open('runtpu.v', 'w') as dest_file:
    pyrtl.importexport.output_to_verilog(dest_file, initialize_registers=False)